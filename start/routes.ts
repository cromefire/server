...
if (Env.get('IS_DASHBOARD_ENABLED') !== 'false') {
  Route.group(() => {
    ...
    // Reset password
    Route.get('forgot', async ({ view }) => view.render('dashboard.forgotPassword')).middleware(
      'guest',
    );
...
Route.get('new', async ({ response, view }) => {
...
Route.get('import', async ({ view }) => view.render('others.import'));

// Legal documents
Route.get('terms', async ({ response }) => response.redirect('/terms.html'));
